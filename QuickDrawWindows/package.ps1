...
Get-ChildItem -Path $TargetDir | % { Copy-Item -Path $_.FullName -Destination $tmpDir -Recurse -Force -Exclude @("*.runtimeconfig.dev.json","*.zip",".DS_Store","language-server-log.txt")};
